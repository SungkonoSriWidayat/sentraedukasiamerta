...
export async function POST(req: NextRequest) {
  try {
    ...
    // Keamanan tambahan: Pastikan tutor adalah pemilik kelas
    const kelas = await Class.findOne({ _id: classId, tutorId: decoded.id });
    if (!kelas) {
        return NextResponse.json({ success: false, message: 'Anda tidak memiliki akses ke kelas ini.' }, { status: 403 });
    }
